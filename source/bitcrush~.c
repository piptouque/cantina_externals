...
{
    x->i_bit_depth = bit_depth_arg < 1. ? 1 : bit_depth_arg > 32. ? 32 : (int)round(bit_depth_arg);
}
...
void* bitcrush_tilde_new(t_floatarg bit_depth_arg, t_floatarg crush_arg)
{
    ...
    // inlet
    x->x_i_in_bit_depth = inlet_new(
            &x->x_obj,
            &x->x_obj.ob_pd,
            &s_float,
            gensym("bit_depth")
                                   );
    x->x_f_in_crush = inlet_new(
            &x->x_obj,
            &x->x_obj.ob_pd,
            &s_float,
            gensym("crush")
...
{
    bitcrush_tilde_class = class_new(
            gensym("bitcrush~"),
            (t_newmethod)bitcrush_tilde_new,
            (t_method)bitcrush_tilde_free,
            sizeof(t_bitcrush_tilde),
            CLASS_DEFAULT,
    ...
    class_addmethod(
            bitcrush_tilde_class,
            (t_method)bitcrush_tilde_bit_depth,
            gensym("bit_depth"),
            A_DEFFLOAT,
            0
    );
